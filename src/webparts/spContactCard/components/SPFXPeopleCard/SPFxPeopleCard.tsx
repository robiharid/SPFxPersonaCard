...
const PROFILE_IMAGE_URL: string =
  "/_layouts/15/userphoto.aspx?size={0}&accountname={1}";

export default class SPFxPeopleCard extends React.PureComponent<
  IPeopleCardProps,
  IPeopleCardState
> {
  ...
  public componentDidMount(): any {
    ...
    const personaImgUrl = Text.format(
      PROFILE_IMAGE_URL,
      size,
      this.props.email
    );
    ...
    this.loadSPComponentById(LIVE_PERSONA_COMPONENT_ID).then(
      (sharedLibrary: any) => {
  ...
  private defaultContactCard() {
    return React.createElement<IPersonaProps>(
      Persona,
      {
  ...
  private getMd5HashForUrl(url: string) {
    return new Promise((resolve, reject) => {
      this.loadSPComponentById(MD5_MODULE_ID)
        .then((library: any) => {
